...
export const TaskDetail: React.FC = () => {
  ...
  const handleAddSubtask = (e: React.FormEvent) => {
      ...
      } else if (task.status === TaskStatus.TODO && updatedSubtasks.some(s => s.completed)) {
           statusUpdate = { status: TaskStatus.IN_PROGRESS };
      }
  ...
  const toggleSubtask = (subtaskId: string) => {
      ...
      if (updatedSubtasks.length > 0) {
          ...
          if (allCompleted && task.status !== TaskStatus.DONE) {
              ...
              // Not all done, but parent was Done -> Revert to In Progress
              statusUpdate = { status: TaskStatus.IN_PROGRESS };
          } else if (anyCompleted && task.status === TaskStatus.TODO) {
              // Started working on subtasks -> Move to In Progress
              statusUpdate = { status: TaskStatus.IN_PROGRESS };
          }
      }
  ...
  const deleteSubtask = (subtaskId: string) => {
      ...
      // If remaining tasks are all done, mark as done if there are any tasks left
      let statusUpdate = {};
      if (updatedSubtasks.length > 0) {
          const allCompleted = updatedSubtasks.every(s => s.completed);
          if (allCompleted && task.status !== TaskStatus.DONE) {
             ...
             <div className="flex items-center gap-2">
                 <div className="relative">
                     <button 
                        ...
                        className={`p-1.5 hover:bg-gray-100 rounded transition-colors ${task.reminder ? 'text-indigo-600' : 'text-gray-400 hover:text-indigo-600'}`}
                        title="Set Reminder"
                     >
                 ...
                 <button 
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 text-gray-600 hover:bg-gray-100 rounded text-xs font-semibold transition-all"
                 ...
                 <button 
                   ...
                   className="flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-xs font-semibold rounded-full hover:shadow-md transition-all disabled:opacity-50"
                 >
                    {isGenerating ? <Loader2 size={12} className="animate-spin"/> : <Sparkles size={12} />}
                    {isGenerating ? 'Thinking...' : 'AI Assist'}
                 </button>
                 <button onClick={handleDelete} className="p-1.5 hover:bg-red-50 text-gray-400 hover:text-red-600 rounded transition-colors" title="Delete Task">
                     <Trash2 size={20} />
                 </button>
                 <button onClick={() => setActiveTaskId(null)} className="p-1.5 hover:bg-gray-100 rounded text-gray-400 hover:text-gray-600">
                     <X size={20} />
                 </button>
             </div>
        </div>
