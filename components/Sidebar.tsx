...
export const Sidebar: React.FC = () => {
  const { spaces, lists, activeSpaceId, activeListId, setActiveSpaceId, setActiveListId, createSpace, updateSpace, deleteSpace, addModule, deleteList, notifications, markNotificationRead, clearAllNotifications, setActiveTaskId, logout } = useContext(StoreContext);
  ...
  return (
    <div className="w-64 bg-gray-50 border-r border-gray-200 flex flex-col flex-shrink-0 h-full relative">
        ...
        <div className="px-2 mb-6">
            <button 
                ...
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors
                    ${activeSpaceId === DASHBOARD_VIEW_ID ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-700 hover:bg-gray-100'}
                `}
            >
                        ...
                        <div className="group flex items-center gap-1 pr-1">
                            <button
                                ...
                                className={`flex-1 flex items-center gap-2 px-2 py-1.5 rounded-md text-sm transition-colors
                                    ${isActive ? 'text-indigo-700 font-medium bg-indigo-50/50' : 'text-gray-600 hover:bg-gray-100'}
                                `}
                            >
                                <div 
                        ...
                        {!isCollapsed && (
                            <div className="ml-7 space-y-0.5 border-l border-gray-200 pl-2">
                                {spaceLists.map(list => (
                                    <div key={list.id} className="relative group/item w-full flex items-center">
                                        <button
                                            ...
                                            className={`flex-1 flex items-center gap-2 px-2 py-1.5 rounded-md text-sm transition-colors
                                                ${activeListId === list.id ? 'bg-gray-200 text-gray-900 font-medium' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'}
                                            `}
                                        >
                                        ...
                                        <button 
                                            ...
                                            className={`p-1.5 text-gray-300 hover:text-red-600 hover:bg-red-50 rounded transition-all flex-shrink-0 ${activeListId === list.id ? 'opacity-100' : 'opacity-0 group-hover/item:opacity-100'}`}
                                            title="Delete Module"
                                        >
                                            <Trash2 size={12} />
                                        </button>
                                ...
                                <button 
                                    ...
                                    className="w-full flex items-center gap-2 px-2 py-1.5 rounded-md text-xs text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors"
                                >
      ...
      <div className="p-4 border-t border-gray-200 bg-white space-y-2 shrink-0">
        <button className="flex items-center gap-3 w-full text-sm text-gray-600 hover:text-gray-900 px-2 py-1.5 rounded hover:bg-gray-50">
            <Search size={18} />
            Search
        </button>
        <div className="relative">
            <button 
                ...
                className={`flex items-center gap-3 w-full text-sm text-gray-600 hover:text-gray-900 px-2 py-1.5 rounded hover:bg-gray-50 ${showNotifications ? 'bg-gray-100' : ''}`}
            >
                <div className="relative">
                    <Bell size={18} />
                    {unreadCount > 0 && (
        ...
        <button 
            onClick={logout}
            className="flex items-center gap-3 w-full text-sm text-rose-600 hover:text-rose-800 px-2 py-1.5 rounded hover:bg-rose-50 mt-2 border-t border-gray-100 pt-3"
        >
            <LogOut size={18} />
            Sign Out
        </button>
      ...
      {editingSpaceId && (
          <div className="absolute top-0 left-0 w-full h-full bg-white/95 backdrop-blur-sm z-30 flex flex-col p-4 animate-[fadeIn_0.2s_ease-out]">
              ...
              <form onSubmit={handleUpdateSpace} className="flex-1 flex flex-col min-h-0">
                  ...
                  <button 
                    ...
                    className="w-full py-2.5 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-700 disabled:opacity-50 transition-colors shadow-sm mb-6 shrink-0"
                  >
                      Save Changes
                  </button>

                  <div className="mt-auto border-t border-gray-200 pt-6 shrink-0">
      ...
      {isCreatingSpace && (
          <div className="absolute top-0 left-0 w-full h-full bg-white/95 backdrop-blur-sm z-10 flex flex-col p-4 animate-[fadeIn_0.2s_ease-out]">
              ...
              <form onSubmit={handleCreateSpace} className="flex-1 flex flex-col min-h-0">
                  <div className="mb-6 shrink-0">
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Space Name</label>
                      <input 
                        autoFocus
                              ...
                              >
                                  ...
                                  <div>
                                      <div className={`text-sm font-semibold flex items-center gap-2 ${selectedModules.has(module.type) ? 'text-indigo-900' : 'text-gray-700'}`}>
                                          <module.icon size={14} /> {module.label}
                                      </div>
                  ...
                  <button 
                    ...
                    className="w-full py-2.5 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-700 disabled:opacity-50 transition-colors shadow-sm shrink-0"
                  >
                      Create Space
                  </button>
              </form>
      ...
      {addingModuleSpaceId && (
          <div className="absolute top-0 left-0 w-full h-full bg-white/95 backdrop-blur-sm z-20 flex flex-col p-4 animate-[fadeIn_0.2s_ease-out]">
              ...
              <form onSubmit={handleAddModule} className="flex-1 flex flex-col min-h-0">
                  <div className="mb-6 shrink-0">
                  ...
                  <div className="mb-6 flex-1 overflow-y-auto min-h-0 pr-1">
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-3 sticky top-0 bg-white/95 backdrop-blur-sm z-10 py-1">Select Type</label>
                      <div className="space-y-2">
                          {availableModules.map(module => (
                              <div 
                                key={module.type}
